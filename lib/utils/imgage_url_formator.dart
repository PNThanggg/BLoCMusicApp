...
String formatImgURL(String imgURL, ImageQuality quality) {
  ImageSource source;
  if (imgURL.contains('youtube') || imgURL.contains('ytimg') || imgURL.contains('googleusercontent')) {
    ...
    case ImageSource.melon:
      {
        switch (quality) {
          case ImageQuality.low:
            {
              return imgURL.replaceAll('resize/350/quality', 'resize/250/quality');
            }
          case ImageQuality.medium:
            {
              return imgURL.replaceAll('resize/350/quality', 'resize/400/quality');
            }
          case ImageQuality.high:
            {
              return imgURL.replaceAll('resize/350/quality', 'resize/500/quality');
            }
        }
      }
    default:
...
String formatYtImgURL(String imgURL, ImageQuality quality) {
  ...
  switch (quality) {
    case ImageQuality.low:
      {
        return imgURL.replaceAll('maxresdefault', 'mqdefault').replaceAll(pattern, 'w200-h200');
      }
    case ImageQuality.medium:
      {
        return imgURL.replaceAll('maxresdefault', 'sddefault').replaceAll(pattern, 'w400-h400');
      }
    case ImageQuality.high:
      {
        return imgURL.replaceAll('maxresdefault', 'maxresdefault').replaceAll(pattern, 'w600-h600');
      }
  }
}
