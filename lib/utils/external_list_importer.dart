...
abstract class ExternalMediaImporter {
  const ExternalMediaImporter();
  ...
  static Stream<ImporterState> ytmPlaylistImporter(String url) async* {
    ...
    if (playlistID != null) {
      try {
        ...
        List<MediaItemModel> mediaItems = fromYtSongMapList2MediaItemList(playlistMeta['songs']);
        ...
        if (tracks.isNotEmpty) {
          ...
          for (var (e as Map) in tracks) {
            try {
              ...
              artists = (e['track']['artists'] as List).map((e) => e['name']).toList().join(", ");
      ...
      try {
        ...
        if (tracks.isNotEmpty && albumTitle.isNotEmpty) {
          for (var (e as Map) in tracks) {
            ...
            artists = (e['artists'] as List).map((e) => e['name']).toList().join(", ");
