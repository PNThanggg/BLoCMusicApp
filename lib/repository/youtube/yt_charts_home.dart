...
Future<List<ChartModel>> fetchTrendingVideos() async {
  ...
  var response = await http.get(Uri.parse('https://charts.youtube.com/charts/TrendingVideos/gb'));
  ...
  response = await http.post(
    Uri.parse('https://charts.youtube.com/youtubei/v1/browse?alt=json&key=$apiKey'),
    headers: headers,
  ...
  if (response.statusCode == 200) {
    ...
    List<dynamic> data = json.decode(response.body)["contents"]['sectionListRenderer']["contents"][0]
    ...
    AppDatabaseService.putChart(chartModels[0]);
    log("Trending Charts: ${chartModels[0].chartItems.length} items loaded", name: "Trending YT Charts");
    return chartModels;
  } else {
    final chart = await AppDatabaseService.getChart("Trending Videos");
    if (chart != null) {
      log("Trending Charts: ${chart.chartItems.length} items loaded from cache", name: "Trending YT Charts");
      return [chart];
    }
