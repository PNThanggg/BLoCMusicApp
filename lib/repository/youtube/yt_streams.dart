...
String getUrl(int option) => "https://www.youtube.com/youtubei/v1/player?key=${allKeys[option]}&prettyPrint=false";
...
class YtStreams {
  ...
  factory YtStreams.fromJson(json) {
    final availableAudioFormats =
        (json["streamingData"]["adaptiveFormats"] as List).where((item) => item['mimeType'].contains("audio"));
    return YtStreams(playable: true, audioFormats: availableAudioFormats.map((item) => Audio.fromJson(item)).toList());
  }
  ...
  Audio get highestBitrateMp4aAudio =>
      audioFormats.lastWhere((item) => item.itag == 140 || item.itag == 139, orElse: () => highestBitrateOpusAudio);

  Audio get highestBitrateOpusAudio =>
      audioFormats.lastWhere((item) => item.itag == 251 || item.itag == 250, orElse: () => highestBitrateMp4aAudio);

  Audio get lowQualityAudio =>
      audioFormats.lastWhere((item) => item.itag == 249 || item.itag == 139, orElse: () => audioFormats.last);

  List<Audio> get sortByBitrate {
    final audioFormatsCopy = audioFormats.toList();
    audioFormatsCopy.sort((audio1, audio2) => audio1.bitrate.compareTo(audio2.bitrate));
...
class Audio {
  ...
  factory Audio.fromJson(json) => Audio(
      audioCodec: (json["mimeType"] as String).contains("mp4a") ? Codec.mp4a : Codec.opus,
      itag: json['itag'],
