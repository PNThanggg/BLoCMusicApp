...
class YouTubeServices {
  ...
  static const String _searchAuthority = 'www.youtube.com';
  ...
  Future<Map?> formatVideoFromId({
    ...
    final Map? response = await formatVideo(
      video: vid,
      quality: await AppDatabaseService.getSettingStr(
            'ytQuality',
            defaultValue: 'Low',
          ) ??
          'Low',
    ...
    try {
      ...
      final List finalResult = shelfRenderer.map((element) {
        ...
        if (playlistItems.isNotEmpty) {
          ...
          log("got null in getMusicHome for '${element['title']['runs'][0]['text']}'", name: "YoutubeAPI");
          return null;
        }
      }).toList();
  ...
  Future<List> getSearchSuggestions({required String query}) async {
    ...
    try {
      final Response response = await get(link, headers: _headers);
      if (response.statusCode != 200) {
        return [];
      }
  ...
  Future<Map?> formatVideo({
    ...
    if (getUrl) {
      // check cache first
      if (checkCache) {
        final ytCache = await AppDatabaseService.getYtLinkCache(video.id.value);
        if (ytCache != null) {
          if ((DateTime.now().millisecondsSinceEpoch ~/ 1000) + 350 > ytCache.expireAt) {
            // cache expired
            urls = await getUri(video);
          } else {
            // giving cache link
            log('cache found for ${video.id.value}', name: "YoutubeAPI");
            urls = [quality == 'High' ? ytCache.highQURL : (ytCache.lowQURL ?? ytCache.highQURL)];
          }
        } else {
          //cache not present
          urls = await getUri(video);
  ...
  Future<List<Map>> fetchSearchResults(String query, {bool playlist = false}) async {
    if (playlist) {
      final List<SearchResult> searchResults = await yt.search.searchContent(query, filter: TypeFilters.playlist);
