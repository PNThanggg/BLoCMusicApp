...
abstract class SaavnAPI {
  const SaavnAPI();

  static Map<String, String> _headers = {};
  static const String _baseUrl = 'www.jiosaavn.com';
  static const String _apiStr = '/api.php?_format=json&_marker=0&api_version=4&ctx=web6dot0';
  static final Map<String, String> _endpoints = {
    ...
    'entityRadio': '__call=web radio.createEntityStation',
  ...
  static Future<Response> _getResponse(String params, {bool usev4 = false}) async {
    String param = params;
    if (!usev4) {
      param.replaceAll('&api_version=4', '');
    }
    Uri url = Uri.parse('https://$_baseUrl$_apiStr&$param');
    ...
    return get(url, headers: _headers).onError((error, stackTrace) {
      return Response(
        {
          'status': 'failure',
  ...
  static Future<Map> _querySongsSearch(String queryText, {int maxResults = 5}) async {
    final String apiUrl =
    ...
    if (response.statusCode == 200) {
      ...
      return {"songs": await formatSongsResponse(getMain, "song").then((value) => value), "error": ""};
    } else {
      return {"no": "error"};
    }
  }
  ...
  static Future<Map> _getSongFromToken(
    String token,
    String type, {
    int n = 10,
    int p = 1,
    ...
    if (n == -1) {
      final String params = "token=$token&type=$type&n=5&p=$p&${_endpoints['fromToken']}";
      try {
        final res = await _getResponse(params);
        if (res.statusCode == 200) {
          final Map getMain = json.decode(res.body) as Map;
          final String count = getMain['list_count'].toString();
          final String params2 = "token=$token&type=$type&n=$count&p=$p&${_endpoints['fromToken']}";
          final res2 = await _getResponse(params2);
      ...
      final String params = "token=$token&type=$type&n=$n&p=$p&${_endpoints['fromToken']}";
      try {
        final res = await _getResponse(params);
        if (res.statusCode == 200) {
          final Map getMain = json.decode(res.body) as Map;
  ...
  static Future<List<Map>> _fetchSearchResults(String searchQuery) async {
    ...
    final res = await _getResponse(params, usev4: false);
    try {
      if (res.statusCode == 200) {
  ...
  static Future<Map> fetchSongSearchResults({
    ...
    final String params = "p=$page&q=$searchQuery&n=$count&${_endpoints['getResults']}";

    try {
      final res = await _getResponse(params, usev4: true);
      if (res.statusCode == 200) {
        final Map getMain = json.decode(res.body) as Map;
  ...
  static Future<List> _getTopSearches() async {
    final response = await get(Uri.parse("https://www.jiosaavn.com/api.php?__call=content.getTopSearches"));
    if (response.statusCode == 200) {
  ...
  static Future<Map<String, dynamic>> _fetchArtistDetails(
    ...
    if (response.statusCode == 200) {
      ...
      final List albums = await formatAlbumResponse(albumResponseList, 'albumSearched');
      return {
  ...
  static Future<Map<String, dynamic>> _fetchPlaylistDetails(
    ...
    final param = '${_endpoints['fromToken']!}&token=$token&type=playlist&n=$n&p=$p';
    final response = await _getResponse(param, usev4: true);
    if (response.statusCode == 200) {
      final Map getMain = json.decode(response.body) as Map;
      // log(getMain.toString());
