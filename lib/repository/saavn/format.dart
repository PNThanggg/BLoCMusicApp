...
String decode(String input) {
  ...
  final String decoded =
      utf8.decode(decrypted).replaceAll(RegExp(r'\.mp4.*'), '.mp4').replaceAll(RegExp(r'\.m4a.*'), '.m4a');
...
Future<Map> formatSingleSongResponse(Map response) async {
  ...
  try {
    ...
    return {
      ...
      'url':
          (response['encrypted_media_url']?.toString() ?? response['more_info']?['encrypted_media_url']?.toString()) !=
                  null
...
Future<Map> formatSinglePlaylistResponse(Map response) async {
  try {
    return {
      ...
      'artist':
          (response['artist_name']?.join(', ')?.toString().unescape() ?? response['extra']?.toString().unescape()),
      'album_artist': response['more_info'] == null ? response['music'] : response['more_info']['music'],
      'image': getImageUrl(response['image'].toString()),
...
Future<Map> formatSingleArtistResponse(Map response) async {
  try {
    return {
      ...
      'type': response['type'],
      'album':
          response['title'] == null ? response['name'].toString().unescape() : response['title'].toString().unescape(),
      'language': response['language']?.toString().capitalize() ??
          response['dominantLanguage'].toString().unescape().capitalize(),
      'genre': response['language'].toString().capitalize(),
      'artistId': Uri.parse((response['perma_url']?.toString() ??
                  (response['url']?.toString() ?? response['urls']?['songs']?.toString().unescape())) ??
              '')
          .pathSegments
          .last,
      'artistToken':
          response['url'] == null ? response['perma_url'].toString() : response['url'].toString().split('/').last,
      'subtitle': response['subtitle']?.toString().unescape() ??
