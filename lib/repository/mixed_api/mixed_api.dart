...
import '../Saavn/saavn_api.dart';
import '../youtube/yt_music_api.dart';

class MixedAPI {
  const MixedAPI();

  static bool _selectBestString(String source, String first, String second) {
    final int a = fw.ratio(source, first);
    final int b = fw.ratio(source, second);
    log("Confidence for string-1($first): $a and string-2($second): $b", name: "MixedAPI");
    return (b > a) ? false : true;
  }

  static Future<MediaItemModel?> getYtTrackByMeta(
    String songName, {
    useStringMatcher = true,
  }) async {
    final ytItems = await YtMusicService().search(songName, filter: "songs");
    List<MediaItemModel> mediaItems = fromYtSongMapList2MediaItemList(ytItems[0]['items'] as List);
      ...
      if (useStringMatcher) {
        ...
        final idx = (r1 > r2)
            ...
            : fw
                ...
                .index;
        log(fw.partialRatio(songName, titles[idx]).toString(), name: "MixedAPI");
        log(fw.ratio(songName, titles[idx]).toString(), name: "MixedAPI");
        if (fw.ratio(songName.trim(), titles[idx]) > 80 || fw.partialRatio(songName.trim(), titles[idx]) > 80) {
  ...
  static Future<MediaItemModel?> _getTrackMixed(String songName) async {
    ...
    if (ytItems.isEmpty && (jsItems.isEmpty || jsItems['songs'].toList().isEmpty)) {
    ...
    if (jsItems["songs"].isNotEmpty) {
      List<String> titles = (jsItems['songs'] as List).map((e) => "${e['title']} ${e['artist']}").toList();
    ...
    if (jsTitle != null && ytTitle != null) {
      final slct = _selectBestString(songName, jsTitle.trim().toLowerCase(), ytTitle.trim().toLowerCase());
      log(slct ? "$jsTitle from JIOSaavn" : "$ytTitle from Youtube", name: "MixedAPI");
      return slct ? fromSaavnSongMap2MediaItem(jsItem!) : fromYtSongMap2MediaItem(ytItem!);
    } else {
      if (jsTitle != null) {
