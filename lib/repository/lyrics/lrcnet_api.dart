  ...
  if (id != null) {
    ...
    try {
      ...
      if (lyrics.lyricsSynced == null) {
        final Lyrics temp = await searchSingleLRCNetLyrics(title, artist: artist, album: album);
        final int ratio = fw.ratio(
...
Future<Lyrics> getLRCNetLyrics(String title, String artist, String album, String duration) async {
// [IN]
// Field	      Required	Type	  Description
...
// }
  ...
  final response = await http
      .get(Uri.parse("$lrcURL$lrcGet?track_name=$title&artist_name=$artist&album_name=$album&duration=$duration"));
...
// plainLyrics and syncedLyrics.
  ...
  final String fields =
      (trackName != null ? "track_name=$trackName" : "") + (artistName != null ? "&artist_name=$artistName" : "");
...
Future<Lyrics> searchSingleLRCNetLyrics(
  ...
  Lyrics lyrics;
  Lyrics? synced;
  final List<Lyrics> lyricsList = await searchLRCNetLyrics(q, artistName: artist, albumName: album);
  ...
  if (lyricsList.isNotEmpty) {
    ...
    final List<Lyrics> syncedList = lyricsList.where((element) => element.lyricsSynced != null).toList();
