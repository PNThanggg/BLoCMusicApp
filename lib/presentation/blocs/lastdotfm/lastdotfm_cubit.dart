...
import '../../../repository/last_fm/lastfmapi.dart';
import '../../../repository/last_fm/models/scrobble_track.dart';
...
class LastdotfmCubit extends Cubit<LastdotfmState> {
  ...
  BehaviorSubject<MediaItemModel> playedMedia = BehaviorSubject<MediaItemModel>.seeded(mediaItemModelNull);
  ...
  Future<void> songTimeTracker() async {
    ...
    scrobbleSub = playerCubit.progressStreams.listen((event) {
      if (playerCubit.appMusicPlayer.audioPlayer.playing &&
          event.currentPlaybackState.processingState == ProcessingState.ready) {
        if (lastPlayed != playerCubit.appMusicPlayer.currentMedia || !stopwatch.isRunning) {
          if (stopwatch.isRunning) {
            stopwatch.stop();
            stopwatch.reset();
          }
          stopwatch.start();
          lastPlayed = playerCubit.appMusicPlayer.currentMedia;
        } else if ((stopwatch.elapsed.inSeconds > 30 ||
                (stopwatch.elapsed.inSeconds /
                        (playerCubit.appMusicPlayer.currentMedia.duration ??
                                const Duration(
                                  hours: 1,
                                ))
                            .inSeconds) >
                    0.5) &&
            playerCubit.appMusicPlayer.currentMedia == lastPlayed &&
            playerCubit.appMusicPlayer.currentMedia != playedMedia.value) {
          playedMedia.add(playerCubit.appMusicPlayer.currentMedia);
          log("Scrobbling: ${playerCubit.appMusicPlayer.currentMedia.title}", name: "Last.FM");
      ...
      } else if (lastPlayed != playerCubit.appMusicPlayer.currentMedia) {
        stopwatch.stop();
        stopwatch.reset();
      } else {
        stopwatch.stop();
  ...
  Future<void> initializeFromDB() async {
    ...
    final username = await AppDatabaseService.getApiTokenDB(GlobalStrConsts.lFMUsername);
    final apiKey = await AppDatabaseService.getApiTokenDB(GlobalStrConsts.lFMApiKey);
    final apiSecret = await AppDatabaseService.getApiTokenDB(GlobalStrConsts.lFMSecret);
    final session = await AppDatabaseService.getApiTokenDB(GlobalStrConsts.lFMSession);
    ...
    if (apiKey != null &&
      ...
      if (session != null && session.isNotEmpty) {
        ...
        emit(LastdotfmIntialized(apiKey: apiKey, apiSecret: apiSecret, sessionKey: session, username: username));
      }
    }
    startUpCheck();
  ...
  Future<void> fetchSessionkey({required String token, required String secret, required String apiKey}) async {
    try {
      final sessionMap = await LastFmAPI.fetchSessionKey(token);
      final session = sessionMap["key"]!;
      final name = sessionMap["name"]!;
      AppDatabaseService.putApiTokenDB(GlobalStrConsts.lFMUsername, name, "0");
      AppDatabaseService.putApiTokenDB(GlobalStrConsts.lFMSecret, secret, "0");
      AppDatabaseService.putApiTokenDB(GlobalStrConsts.lFMApiKey, apiKey, "0");
      AppDatabaseService.putApiTokenDB(GlobalStrConsts.lFMSession, session, "0");
  ...
  Future<String> startAuth({required String apiKey, required String secret}) async {
  ...
  Future<void> remove() async {
    ...
    AppDatabaseService.putApiTokenDB(GlobalStrConsts.lFMSecret, "", "0");
    AppDatabaseService.putApiTokenDB(GlobalStrConsts.lFMApiKey, "", "0");
    AppDatabaseService.putApiTokenDB(GlobalStrConsts.lFMSession, "", "0");
    AppDatabaseService.putApiTokenDB(GlobalStrConsts.lFMUsername, "", "0");
  }
  ...
  Future<bool> scrobble(MediaItemModel mediaItem) async {
    final shouldScrobble =
        await AppDatabaseService.getSettingBool(GlobalStrConsts.lFMScrobbleSetting, defaultValue: false);
    ...
    if (shouldScrobble ?? false) {
      ...
      trackList.add(track);
      try {
        if (LastFmAPI.initialized && mediaItem != mediaItemModelNull && durationMin < 15 && durationSec > 30) {
          final response = await LastFmAPI.scrobble(trackList);
          log('Scrobble response: $response', name: 'LastFM API');
          return response;
        }
  ...
  void lFMCacheTrack(List<ScrobbleTrack> trackList) {
    ...
    AppDatabaseService.getAPICache(GlobalStrConsts.lFMTrackedCache).then((value) {
      if (value != null && value != "null") {
        log("Cache found: ${trackListMap.toString()}", name: "Last.FM");
        final trackList2 = value as List;
        trackList2.addAll(trackListMap);
        AppDatabaseService.putAPICache(GlobalStrConsts.lFMTrackedCache, trackList2.toString());
      } else {
        log("No cache found", name: "Last.FM");
        AppDatabaseService.putAPICache(GlobalStrConsts.lFMTrackedCache, trackListMap.toString());
      }
    });
  }

  Future<List<ScrobbleTrack>> getLFMTrackedCache() async {
    final trackList = await AppDatabaseService.getAPICache(GlobalStrConsts.lFMTrackedCache);
    await AppDatabaseService.putAPICache(GlobalStrConsts.lFMTrackedCache, "null");
  ...
  Future<MediaPlaylist> getRecommendedTracks() async {
    ...
    for (var track in response['playlist']) {
      ...
      final mediaItem = await MixedAPI.getYtTrackByMeta("$title ${artists.join(' ')}");
      if (mediaItem != null) {
